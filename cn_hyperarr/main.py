...
def check_all_regions(vector_conf, backend=None, verbose=False):
    r"""
    Checks the acyclicity for all regions in the hyperplane arrangement generated by the vector configuration.
    ...
    Testing an arrangement that is not always congruence normal::
    ...
    for i in range(len(R)):
        ...
        nb_shards.add(forcings[i][1])
    return acyclic_dict,nb_shards
...
def report_on_congnorm(vector_conf, cuntz_index, folder):
    ...
    The standard output of the computations are written is a file named
...
def filter_matroid_arrangements_into_families(indices=None,excluded_nb=[],parallel_bound=28):
    r"""
    Filters the arrangements into the three infinite families or "others".
    """
    print("Loading Cuntz's list...")
    ...
    for new_index in range(nb):
        ...
        vl = arrangements_list[cuntz_index]
        nb_v = len(vl)
        if nb_v not in needed_ns:
            needed_ns += [nb_v]
        vector_conf = VectorConfiguration(vl,backend='normaliz')
        vc_list += [vector_conf]
    
    print("The needed numbers are {}".format(needed_ns))

    with multiprocessing.Pool(processes=8) as pool:
        odd = [(_,"normaliz") for _ in needed_ns if _ % 2 == 1]
        even = [(_,"normaliz") for _ in needed_ns if _ % 2 == 0]
        print("Computing polygons in parallel")
        p_mat = pool.map(polygons_matroid,even)
        polygons = {even[index][0]:p_mat[index] for index in range(len(even))}
        print("Computing polygons inf in parallel")        
        p_inf_mat = pool.map(polygons_infinity_matroid,odd)
        polygons_inf = {odd[index][0]:p_inf_mat[index] for index in range(len(odd))}
    ...
    for new_index in range(nb):
        ...
        if nb_v not in excluded_nb:
            if nb_v % 2 == 0:
                input_data = tuple([vector_conf,polygons[nb_v],0])
            else:
                input_data = tuple([vector_conf,0,polygons_inf[nb_v]])
            the_inputs += [input_data]
        else:
            input_data = tuple([0,0,0])
            the_inputs += [input_data]

    with multiprocessing.Pool(processes=8) as pool:
        print("Launching the types fetching")
        types = pool.map(get_mat_type,the_inputs)

    # types = list(map(get_mat_type,the_inputs))
    ...
    return list_poly,list_poly_inf,other
...
def classify_cuntz():
    ...
    for index in range(nb):
        vl = arrangements_list[index]
        vector_conf = VectorConfiguration(vl,backend='normaliz')
        mat = vector_conf.underlying_matroid()
        new = True
        for rep in dict_classes.keys():
            if mat.is_isomorphic(rep):
                new = False
                dict_classes[rep] += [index]
                break
        if new:
            dict_classes[mat] = [index]
